...
function placeFudaInRows(fieldElement, fudaIds) { /* ... (省略 - 変更なし) ... */ }
...
// 4a. ★ タップ（クリック）イベントハンドラ (タップ方式版)
function handleFieldClick(e) {
    const clickedElement = e.target;
    const clickedFuda = clickedElement.closest('.fuda'); // タップされた札自体
...
// 5. タイマー開始 (★ 操作説明テキスト変更)
function startTimer(duration) { if (currentGameTimer) clearInterval(currentGameTimer); timerSelect.disabled = true; updateInstructionText("暗記中 (自陣の札をタップして選択、再度タップで配置)"); let timer = duration; currentGameTimer = setInterval(() => { const minutes = Math.floor(timer / 60); const seconds = timer % 60; timerDisplay.textContent = `${minutes < 10 ? '0' : ''}${minutes}:${seconds < 10 ? '0' : ''}${seconds}`; if (--timer < 0) { clearInterval(currentGameTimer); currentGameTimer = null; timerDisplay.textContent = "暗記終了！"; timerSelect.disabled = false; enterPracticeMode(); } }, 1000); }
// 6. 暗記練習モード (変更なし)
function enterPracticeMode() { if (selectedFuda) cancelSelection(); const allBaFuda = document.querySelectorAll('#jiJin .fuda, #tekiJin .fuda'); allBaFuda.forEach(fuda => { fuda.classList.add('back'); fuda.style.cursor = 'pointer'; }); document.body.removeEventListener('click', handleFieldClick); document.body.addEventListener('click', handleFlipClick); updateInstructionText("暗記練習中 (裏向きの札をタップで確認)"); }
...
function updateInstructionText(customText = null) {
    ...
    if (selectedFuda) {
        textElement.textContent = "配置したい場所をタップしてください (同じ札か無効な場所タップでキャンセル)"; // タップ版
    } else if (isInManualSetup) {
        textElement.textContent = "手札の札をタップして選択し、自陣または手札に戻したい場所をタップして配置"; // タップ版
    } else if (isTimerRunning) {
        textElement.textContent = "自陣の札をタップして選択し、移動させたい場所をタップして配置"; // タップ版
